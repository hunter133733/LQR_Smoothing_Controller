...
DEFAULT_COST_COEFS = {
    ...
    "y": 5.0,
    "theta": 1.0,
    "v": 0.3, # penalty on control deviation itself
    "w": 0.3,
    "dv": 0.05, # pentalty on change in control deviation
    "dw": 0.10
}


class SmoothedLQRAlgorithm:
    """Finite-horizon time-varying LQR for Dubins trajectory tracking, extended with Ext3 smoothing by penalizing changes in control.
    
        Augmented state:
            xi_t = [delta_z_t, delta_u_{t-1}]
        New control variable:
            nu_t = delta_u_t - delta_u_{t-1}
    """
    ...
    def __init__(
        ...
        #penalty on control deviation itself
        self.Ru = np.diag([self.cost_coefs["v"], self.cost_coefs["w"]]).astype(float)
        self.Rdu = np.diag([self.cost_coefs["dv"], self.cost_coefs["dw"]]).astype(float)

        #augmented state cost
        self.Q_aug = np.block (
            [
                [self.Qz, np.zeros((3,2), dtype=float)],
        ...
        self.L_aug = np.block (
            [
                [self.Lz, np.zeros((3,2), dtype=float)],
                [np.zeros((2,3), dtype=float), self.Ru]
            ]
        ).astype(float)
        ...
        # most recent previous control deviation
        self.delta_u_prev = np.zeros(2, dtype=float)
    ...
    def solve(
        ...
        z_ref: NDArray,
        u_ref: NDArray,
        delta_u_prev0: NDArray | None = None,
        ...
        if delta_u_prev0 is None:
            delta_u_prev = np.zeros(2, dtype=float)
        else:
            delta_u_prev = np.asarray(delta_u_prev0, dtype=float).reshape(2).copy()

        for i in range(n_track):
            z_t = np.asarray(self.dynsys.z_hist[-1, :], dtype=float).reshape(3)

            delta_z = z_t - z_track[i, :]
            delta_z[2] = self._wrap_angle(delta_z[2])

            # Augmented state xi_t = [delta_z_t; delta_u_{t-1}]
            xi_t = np.concatenate([delta_z, delta_u_prev], axis=0)

            # New control variable is increment in control deviation:
            # nu_t = delta_u_t - delta_u_{t-1}
            nu_t = -Ks[i] @ xi_t

            # Recover current control deviation and actual control
            delta_u_t = delta_u_prev + nu_t
            u_t = u_track[i, :] + delta_u_t
            u_t = np.clip(u_t, self.action_min, self.action_max)

            # Keep the stored deviation consistent with clipped action
            delta_u_t = u_t - u_track[i, :]
            delta_u_prev = delta_u_t

            self.dynsys.forward_np(dt=self.dt, ctrl=u_t)

        self.delta_u_prev = delta_u_prev.copy()
    ...
    def compute_gains(self, As: NDArray, Bs: NDArray) -> Tuple[NDArray, NDArray]:
        """
        Compute finite-horizon time-varying LQR gains for the augmented Ext3 system.

        Original error dynamics:
            delta_z_{t+1} = A_t delta_z_t + B_t delta_u_t
        ...
        for i in range(n):
            ...
            A_aug[i] = np.block(
                [
                    [A_t, B_t],
                    [np.zeros((2, 3), dtype=float), np.eye(2, dtype=float)],
                ]
            )
            B_aug[i] = np.vstack([B_t, np.eye(2, dtype=float)])

        Ks = np.zeros((n, 2, 5), dtype=float)
        Ps = np.zeros((n + 1, 5, 5), dtype=float)

        Ps[n] = self.L_aug.copy()
        ...
        for i in range(n - 1, -1, -1):
            ...
            S = self.Rdu + B_t.T @ P_next @ B_t
            Ks[i] = np.linalg.solve(S, B_t.T @ P_next @ A_t)

            Ps[i] = self.Q_aug + A_t.T @ P_next @ A_t - A_t.T @ P_next @ B_t @ Ks[i]
            Ps[i] = 0.5 * (Ps[i] + Ps[i].T)  # numerical symmetry
...
class SmoothedLQRController(ControllerBackend):
    ...
    def __init__(self, config):
        ...
        cost_coefs = {
            ...
            "dv": float(cfg.get("dv_cost", DEFAULT_COST_COEFS["dv"])),
            "dw": float(cfg.get("dw_cost", DEFAULT_COST_COEFS["dw"])),
        }
        ...
        self._algo = SmoothedLQRAlgorithm(
        ...
        # Previous control deviation carried across receding-horizon calls
        self._delta_u_prev = np.zeros(2, dtype=float)
    ...
    def get_action(
        ...
        if self._z_ref is None or self._u_ref is None:  # debugging fallback
        ...
        z_sol, u_sol, _ = self._algo.solve(
            ...
            z_ref=z_ref_win,
            u_ref=u_ref_win,
            delta_u_prev0=self._delta_u_prev,
        )

        if u_sol is not None and u_sol.shape[0] > 0:
            action = np.asarray(u_sol[0, :], dtype=float).reshape(2)
            self._delta_u_prev = action - u_ref_win[0, :]
        else:
